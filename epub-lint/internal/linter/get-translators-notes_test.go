...
func TestGetTranslatorsNotes(t *testing.T) {
	testCases := []getTranslatorsNotesTestCase{
		{
			name: "3 author notes with two lines inbetween each other and starting number is 3",
...
`,
		...
		{
			name: "starting number is 0 and there are no translator's notes meaning no changes to the text",
		...
		{
			name: "starting number is 0 and there is a single translator's note that is encapsulated in parentheses",
...
`,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			updatedText, notes, next := linter.GetTranslatorsNotes(tc.inputText, tc.fileName, tc.noteFileName, tc.startingNumber)
			assert.Equal(t, tc.expectedText, updatedText, "expected text output mismatch")
			assert.Equal(t, tc.expectedNotes, notes, "expected notes mismatch")
			assert.Equal(t, tc.expectedNext, next, "expected next note number mismatch")
		})
	}
}
