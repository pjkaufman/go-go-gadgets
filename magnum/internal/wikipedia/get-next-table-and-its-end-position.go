...
func GetNextTableAndItsEndPosition(sectionHtml string) (string, int) {
	...
	for {
		...
		if attemptNum > maxAttempts {
			logger.WriteFatalf("something went wrong trying to parse out the table from %s, as there were only %d instances of table endings and we are trying to find the %d table ending", sectionHtml, maxAttempts, attemptNum)
		}
	}
