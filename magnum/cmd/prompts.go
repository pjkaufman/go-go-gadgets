...
func selectBookName(series []config.SeriesInfo, includeCompleted bool) string {
	...
	if err != nil {
		logger.WriteFatalf("Book name prompt failed %v", err)
	}
...
func selectBookStatus(currentStatus *config.SeriesStatus) config.SeriesStatus {
	...
	i, _, err := prompt.Run()
	if err != nil {
		logger.WriteFatalf("Book status prompt failed %v\n", err)
	}
...
func selectPublisher(currentPublisher *config.PublisherType) config.PublisherType {
	...
	i, _, err := prompt.Run()
	if err != nil {
		logger.WriteFatalf("Book publisher prompt failed %v", err)
	}
...
func selectSeriesType(currentType *config.SeriesType) config.SeriesType {
	...
	i, _, err := prompt.Run()
	if err != nil {
		logger.WriteFatalf("Book series type prompt failed %v", err)
	}
